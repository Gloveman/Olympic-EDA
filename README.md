...
## 번외: 프로젝트 개인 회고
### 잘했던 점
최적화된 Data Loading: ipynb 파일에서 직접 csv 파일을 불러와 사용하는 대신 최소한의 전처리만 거친 효율적인 dataset load 함수를 설계하여 사용했니다.

Tidy Data로의 전환: 연도별 column으로 펼쳐져 있던 GDP 데이터를 pandas의 melt 함수를 통해 Year 컬럼과 이에 대응되는 GDP 컬럼으로 매핑되도록 통합했습니다. 이를 통해 Olympic dataframe과의 merge가 가능한 형태로 재구조화하여 시계열 분석이 가능하도록 했습니다.

데이터 보강: 원본 데이터에 명시되지 않은 '개최국' 정보를 얻기 위해 개최 도시(City)와 국가 코드(NOC)를 매핑하는 로직을 구현했습니다.

Global Styling: `plt.style_use`와 `sns.set_theme`를 활용해 프로젝트 전반에 통일된 다크 테마와 네온 컬러 팔레트를 적용했습니다. 이는 데이터의 가독성을 높이고 그래프 간 디자인 통일성을 주었습니다.

Insight-Driven Annotation: 그래프 위에 단순 수치 나열이 아닌 **상승률(%)**을 계산하여 명시했습니다. 이는 관찰자가 계산을 거치지 않고도 '개최국 버프'의 위력을 즉각적으로 체감할 수 있게 돕는 장치가 되었습니다.

### 보완할 점
재개최(M 효과의 세분화
데이터를 보다 보니, 올림픽을 처음 유치하는 국가와 두 번째 유치하는 국가(예: 미국, 영국 등)의 에너지는 다를 것임을 깨달았습니다. 이미 인프라가 갖춰진 상태에서의 유치가 성적에 미치는 영향을 별도로 분리하여 분석했다면, **'올림픽 유산의 감가상각'**에 대한 더 깊은 통찰을 얻었을 것입니다.

종목별 분석 보안
종목별 비교 분석에서 전체적인 종목 성격만 다루었는데, 여기서 더 나아가 하계/동계 시즌별 분리 혹은 개최 전후의 구체적인 메달 변동폭을 대조했다면 더 정교한 '전략 종목' 리포트가 되었을 것이라는 아쉬움이 남습니다.

데이터 결측치에 대한 아쉬움 (Age Column)
만약 Age 컬럼의 데이터가 온전했다면, 개최국이 되었을 때 유망주(Youth) 기용 비율이 어떻게 변하는지 분석해보고 싶었습니다. 개최국으로서 단기 성적에 집중했는지, 혹은 미래를 위한 신인 발굴에 집중했는지를 확인하지 못한 점이 향후 보완 과제로 남았습니다.
